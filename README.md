...
1. The RflyMAD dataset is able to be downloaded from [here](https://rfly-openha.github.io/documents/4_resources/dataset.html). And the data formats in `<Exact flight cases>` could be seen at [Generate your own JSON](#25-additional-generate-your-own-json-for-data-topic-selection).

2. The `SIL` and `HIL` are different from `SILwithROS` and `HILwithROS`, please BE ATTENTION ! The `SIL` and `HIL` are simulation data generated by RflySim platform. Although the `SILwithROS` and `HILwithROS` are simulation data, they are generated by RflySim and ROS together, so `SILwithROS` and `HILwithROS` have the same data structure with `real flight data`.
...
|--sub_dataset  | Sub-dataset used in process | 0 for all, 1 for SIL, 2 for HIL, 3 for Real, 4 for SIL with ROS and 5 for HIL woth ROS|
...
|:------------:|:---------:|:------:|
|Flight Information| Contains flight command, fault type and fault parameter | SIL, HIL, SIL/HIL with ROS and Real |
|ULog | Data recorded by autopilot | SIL, HIL, SIL/HIL with ROS and Real |
|Telemetry Log | Information communicated between a multicopter and its corresponding QGroundControl | SIL, HIL, SIL/HIL with ROS and Real|
|Ground Truth Data   | Generated by RflySim platform during the simulation | SIL and HIL  |
|BAG| Generated by the ROS system in real flight |Real flight, SIL/HIL with ROS|
...
- `data_real_ROS.json`. Used for Real flight data and SIL/HIL data with ROS, extract data in `ROS BAG`.
- `data_real_PX4.json`. Used for Real flight data and SIL/HIL data with ROS, extract data in `ULog`.
...
If you want to generate your own `JSON` files when using `SILwithROS` or `HILwithROS`, please use `mode=1` and change the correlated path.
