...
Welcome to use Data processing tools designed for RflyMAD dataset ! With this toolkit, you can select your interested topics and files in RflyMAD dataset, and then generate processed file at certain frequency as you want !!!
...
### 2. Select Data Topic
After choose the data type you want, the next step is to select the message topic in each files, and that is also a key step to prepare the data which are used for research. As mentioned in our [data paper]() and the introduction website of [RflyMAD](https://rfly-openha.github.io/documents/4_resources/dataset.html), each flight within the dataset contains four types of raw data and relevant processes files. They could be described as following:

|Name          |Meaning    |Exists in |
|:------------:|:---------:|:------:|
|Flight Information|RflyMAD original dataset restore path| SIL, HIL and Real |
|ULog |RflyMAD processed data restore path | SIL, HIL and Real |
|Telemetry Log | Sub-dataset used in process | SIL, HIL and Real|
|Ground Truth Data   | Fault type selected in process | SIL and HIL  |
|BAG| Flight status selected in process |Real flight|
...
Before using the function, you need to adjust `mode` and data restore formats, and your own data restore formats need to de adjusted as follows:
```
...
\<Exact flight cases>
    \PX4_path
...
```
When set `mode=0`, the program will generate `JSON` files for PX4 and Ground truth data in SIL and HIL simulation data. For `JSON` files in HIL and SIL have similar formats, you can just choose one of them. When set `mode=1`, `JSON` files for real flight data will be generated, including file for PX4 and Rosbag.
...
Attention:
The first time you run this toolkit may take a long time to generate processed files. For the GTD data files converted from XLSX to CSV is time-consuming and the conversion of ULog to CSV also takes time. 
The converted CSV files will be restored in the original dataset path, they may take up some storage space of your device, but they will speed up the next run. So please consider whether to retain these intermediate files at your discretion.

Note:
The `ULog` and `BAG` data in real flight data have been converted in original `.rar` files. 
